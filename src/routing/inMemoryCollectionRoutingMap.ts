...
export class InMemoryCollectionRoutingMap {
    ...
    public getRangeByEffectivePartitionKey(effectivePartitionKeyValue: string) {
        ...
        if (Constants.EffectiveParitionKeyConstants.MinimumInclusiveEffectivePartitionKey ===
            effectivePartitionKeyValue) {
            return this.orderedPartitionKeyRanges[0];
        }

        if (Constants.EffectiveParitionKeyConstants.MaximumExclusiveEffectivePartitionKey ===
            effectivePartitionKeyValue) {
            return undefined;
        }
    ...
    public getOverlappingRanges(providedQueryRanges: QueryRange | QueryRange[]) {
        ...
        const sortedHigh = this.orderedRanges.map(
            (r) => {
        ...
        for (const queryRange of pqr) {
            ...
            const minIndex = bs.le(sortedLow,
            ...
            const maxIndex = bs.ge(sortedHigh,
                { v: queryRange.max, b: queryRange.isMaxInclusive },
                InMemoryCollectionRoutingMap._vbCompareFunction);
            assert.ok(maxIndex < sortedHigh.length,
            ...
            for (let j = minIndex; j < maxIndex + 1; j++) {
                if (queryRange.overlaps(this.orderedRanges[j])) {
                    minToPartitionRange[this.orderedPartitionKeyRanges[j][Constants.PartitionKeyRange.MinInclusive]] =
