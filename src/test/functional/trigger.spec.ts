...
describe("NodeJS CRUD Tests", function() {
  ...
  describe("validate trigger functionality", function() {
    ...
    it("should do trigger operations successfully with create", async function() {
      ...
      // create document
      const document = await container.items.create({ id: "doc1", key: "value" }, { preTriggerInclude: "t1" });
      assert.equal(document.id, "DOC1t1", "name should be capitalized");
      const document2 = await container.items.create({ id: "doc2", key2: "value2" }, { preTriggerInclude: "t2" });
      assert.equal(document2.id, "doc2", "name shouldn't change");
      const document3 = await container.items.create({ id: "Doc3", prop: "empty" }, { preTriggerInclude: "t3" });
      assert.equal(document3.id, "doc3t3");
      const document4 = await container.items.create(
        { id: "testing post trigger" },
        { postTriggerInclude: "response1", preTriggerInclude: "t1" }
      );
      assert.equal(document4.id, "TESTING POST TRIGGERt1");
      const document5 = await container.items.create({ id: "responseheaders" }, { preTriggerInclude: "t1" });
    ...
    it("should do trigger operations successfully with upsert", async function() {
      ...
      // create document
      const document = await container.items.upsert({ id: "doc1", key: "value" }, { preTriggerInclude: "t1" });
      assert.equal(document.id, "DOC1t1", "name should be capitalized");
      const document2 = await container.items.upsert({ id: "doc2", key2: "value2" }, { preTriggerInclude: "t2" });
      assert.equal(document2.id, "doc2", "name shouldn't change");
      const document3 = await container.items.upsert({ id: "Doc3", prop: "empty" }, { preTriggerInclude: "t3" });
      assert.equal(document3.id, "doc3t3");
      const document4 = await container.items.upsert(
        { id: "testing post trigger" },
        { postTriggerInclude: "response1", preTriggerInclude: "t1" }
      );
      assert.equal(document4.id, "TESTING POST TRIGGERt1");
      const document5 = await container.items.upsert({ id: "responseheaders" }, { preTriggerInclude: "t1" });
      assert.equal(document5.id, "RESPONSEHEADERSt1");
      try {
