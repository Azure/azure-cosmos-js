    ...
    describe("Validate container indexing policy", function () {
        const indexPolicyTest = async function () {
            try {
                ...
                const lazyContainer = database.containers.getContainer(lazyContainerDef.id);
                ...
                const consistentContainer = database.containers.getContainer(consistentContainerDef.id);
                assert.equal(containerDef.indexingPolicy.indexingMode,
    ...
    describe("Validate response headers", function () {
        const createThenReadCollection = async function (database: Database, body: ContainerDefinition) {
            try {
                ...
                const response = await database.containers.getContainer(createdCollection.id).read();
                return response;
            } catch (err) {
                throw err;
            }
