...
describe("NodeJS CRUD Tests", function () {
    ...
    describe("Validate Authorization", function () {
        const setupEntities = async function (isUpsertTest: boolean, client: CosmosClient) {
            ...
            // create container1
            const { body: container1 } = await client.databases.get(db.id)
                .containers.create({ id: "Validate Authorization container" });
            // create document1
            const { body: document1 } = await client.databases.get(db.id)
                .containers.get(container1.id)
                .items.create({ id: "coll1doc1", foo: "bar", key: "value" });
            // create document 2
            const { body: document2 } = await client.databases.get(db.id)
                .containers.get(container1.id)
                .items.create({ id: "coll1doc2", foo: "bar2", key: "value2" });

            // create container 2
            const { body: container2 } = await client.databases.get(db.id)
        ...
        const authorizationCRUDTest = async function (isUpsertTest: boolean) {
            ...
            const { body: successColl1 } = await col1Client.databases.get(entities.db.id)
            ...
            const { body: successDoc } = await col1Client.databases.get(entities.db.id)
                .containers.get(entities.coll1.id)
