...
describe("NodeJS CRUD Tests", function() {
  ...
  describe("Validate Authorization", function() {
    const setupEntities = async function(isUpsertTest: boolean, client: CosmosClient) {
      ...
      const { result: permissionOnColl1 } = await createOrUpsertPermission(
      ...
      const { result: permissionOnDoc2 } = await createOrUpsertPermission(
        client.database(db.id).user(user1.id),
        permission,
        undefined,
        isUpsertTest
      ...
      const { result: permissionOnColl2 } = await createOrUpsertPermission(
