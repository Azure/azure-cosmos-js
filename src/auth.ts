...
export class AuthHandler {
    // TODO: documentClient
    public static async getAuthorizationHeader(documentClient, verb: string, path: string, resourceId:string, resourceType:string, headers: {[key: string]: string | boolean}): Promise<string> {
        if (documentClient.masterKey) {
            ...
            return encodeURIComponent(await AuthHandler.getAuthorizationTokenUsingTokenProvider(documentClient.tokenProvider, {
                verb: verb, path: path, resourceId: resourceId, resourceType: resourceType, headers: headers
            }));
        }
    }
    ...
    private static getAuthorizationTokenUsingMasterKey(
        ...
        headers: { [key: string]: string | boolean }, masterKey: string) {
    ...
    private static getAuthorizationTokenUsingResourceTokens(resourceTokens, path: string, resourceId: string) {
    ...
    private static getAuthorizationTokenUsingTokenProvider(tokenProvider: ITokenProvider, requestInfo: IRequestInfo): Promise<string> {
